...
from interface_dev_tools import InterfaceDevTools
from api_performance_ui import render_api_performance_test_page
from api_security_ui import render_api_security_test_page
...
if st.session_state.pending_tool_scroll:
    scroll_counter = st.session_state.tool_scroll_counter
    components.html(
        f"""
        <script>
        const scrollToToolContent = () => {{
            const anchor = window.parent.document.getElementById('tool-content-anchor');
            if (anchor) {{
        ...
        <div data-scroll-counter="{scroll_counter}"></div>
        """,
        height=0,
    )
    st.session_state.pending_tool_scroll = False
